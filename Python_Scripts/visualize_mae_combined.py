...
def plot_ecs(data):
    ...
    data = data.rename(index=lambda x: re.sub('Mean_target','Time_point',x))
    data = data.rename(index={"Time_point_1": "0.0hr",
                        "Time_point_2": "0.5hr",
                        "Time_point_3": "1.0hr",
                        "Time_point_4": "1.5hr",
                        "Time_point_5": "2.0hr",
    ...
    ax.set_ylabel("MAE values", fontsize=25)
    plt.suptitle(f'Combined Mean MAE (interpolated) across all subjects', fontsize=25)
    plt.yticks(np.arange((min_val), (max_val), 3))
    # ax.legend(fontsize=12)
    plt.yticks(fontsize=25)
    plt.xticks(fontsize=25)
    filename = "all_mean_mae_plot_combined.png"
...
if __name__=="__main__":
    ...
    data = combined.drop(["SID"], axis=1).mean(axis=0)
    print(data.T)
    # data=combined
